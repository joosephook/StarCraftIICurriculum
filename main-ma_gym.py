...
from ma_gym_wrapper import MAGymWrapper
...
class ForwardCurriculum:
    def __init__(self, envs: List[Tuple[StarCraft2Env, int]], patience, switch_callback):
        ...
        for i, (e, steps) in enumerate(envs):
            e.switch = (i+1) != len(envs)
        self.next()
...
if __name__ == '__main__':
    ...
    if config.get("probs", None):
        curriculum = SamplingCurriculum(
            ...
            total_timesteps=sum(train_timesteps),
            p=config.get("probs"),
        ...
        curriculum = ForwardCurriculum(
            [
                (env, steps) for env, steps in zip(train_envs, train_timesteps)
            ],
